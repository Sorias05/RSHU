...
namespace Lab6
{
    internal class Graph
    {
        ...
        //повертає список суміжності у вигляді не орієнтованого графа
        public List<int>[] adjacency_list_unoriented()
        {
            List<int>[] list = new List<int>[VertexCount]; //сюди помістимо результат
            for (int i = 0; i < VertexCount; i++) list[i] = new List<int>();//створюємо всі елементи списку
            foreach (Edge edge in Edges)
            {
                list[edge.From.Number - 1].Add(edge.To.Number - 1);//додаємо в список зв'язок між вершинами
                list[edge.To.Number - 1].Add(edge.From.Number - 1);//і зворотній зв'язок, для представлення неорієнтованого графу
                                                                   // -1 бо у нас вершини починаються з 1-ці а елементи списку з 0-ля
            }
            return list;
        }
        //функція розфарбування графу
        public int[] colorGraph() //повертає масив індексів кольору вершин графа
        {
            List<int>[] adj_list = adjacency_list_unoriented();//отримуємо список суміжності вершин, функція одразу додасть обернені зв'язки, для зручності
            int[] colors = new int[VertexCount];//створимо масив кольорів для їх зберігання
            for (int i = 0; i < VertexCount; i++)//перебираємо всі елементи масиву кольорів
                colors[i] = -1;//присвоюємо кожному елементу -1, означає що у вершини ще колір не вирахуваний
            colors[0] = 1;//колір першої вершини =1
            for (int i = 1; i < VertexCount; i++)//проходимося по всіх вершинах графу і-поточна внршина
            {
                bool[] usedColors = new bool[VertexCount + 1];//оголосимо масив використовуваних кольорів
                foreach (int j in adj_list[i])//перебтраємо по списку суміжності всі зв'язані вершини (j) з вершиною графу і
                {
                    if (colors[j] != -1)//якщо з'єднана вершина розфарбована
                        usedColors[colors[j]] = true;//то помічаємо, що колір, яким розфарбована верщина j використаний
                }
                int k = 1;//присвоюємо k значення 1
                while (usedColors[k]) k++;// рахуємо скільки кольорів будо використано, то і є наш колір вершини
                colors[i] = k;//присвоюємо вершині її індекс кольору
            }
            return colors;//повертаємо масив кольорів вершин }
        }
    }
}
